...
# ====== Hypothesis testing ======
# TODO: run this 1,000 times and check how many times x1, x2, and x3 are included.
...
print(null_hypothesis_test_results)
...
for(i in 1:nrow(regression_demo)){
  models_leave_one_out = list(
    "simple"  = lm(y ~ x1, data = regression_demo[-i,]),
    "medium"  = lm(y ~ x1 + x2, data = regression_demo[-i,]),
    "complex" = lm(y ~ x1 + x2 + x3, data = regression_demo[-i,])
  )
  for(m in c("simple", "medium", "complex")){
    leave_one_out_prediction_error[[m]] = 
      leave_one_out_prediction_error[[m]] + 
      (predict(models_leave_one_out[[m]], newdata = regression_demo[i,] ) - regression_demo[i,"y"])^2
  }
}
leave_one_out_prediction_error

# ===== Fitting error =====
# TODO: Modify the LOOCV code so that models are fit to the whole dataset and prediction error is measured on the whole dataset.
