...
def generate_allele_counts(allele_frequency, strength_of_assortative_mating, n):
    ...
    assert isinstance(allele_frequency, (int, float)) and 0 < allele_frequency < 1, "Allele frequency must be between 0 and 1"
    assert isinstance(strength_of_assortative_mating, (int, float)) and 0 < strength_of_assortative_mating < 1, "Strength of assortative mating must be between 0 and 1"
...
for i, filename in enumerate(filenames):
    am = np.random.choice([0, 0.3], 1)
    allele_counts = np.apply_along_axis(lambda af: generate_allele_counts(af, am, 1000), axis=0, arr=np.arange(0.1, 1, 0.1))
